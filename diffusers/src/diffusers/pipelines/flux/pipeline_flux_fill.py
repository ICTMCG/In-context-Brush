...
class FluxFillPipeline(
    ...
    def __call__(
        ...
        ###############################
        # # mask latent
        # mask_image = mask_image.to(masked_image.dtype).repeat(1, 3, 1, 1)
        # mask_image = mask_image.to(device)
        # mask_image_latents = self.prepare_mask_latents_true(
        #     mask_image,
        #     batch_size,
        #     num_channels_latents,
        #     num_images_per_prompt,
        ...
        #     generator,
        # )
        mask_pack = torch.nn.functional.interpolate(mask_image, size=(height//2, width//2), mode='bilinear', align_corners=False)
        mask_pack = mask_pack.view(1, 1, 4, height//self.vae_scale_factor, 4, width//self.vae_scale_factor)
        mask_pack = mask_pack.permute(0, 1, 2, 4, 3, 5).contiguous()
        mask_latents = mask_pack.view(1, 16, height//self.vae_scale_factor, width//self.vae_scale_factor)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ########################################
                # blending
                source_latents, _ = self.prepare_image_latents(
                    image,
                    timestep,
                    batch_size * num_images_per_prompt,
                    num_channels_latents,
                ...
                source_latents_unpack = self._unpack_latents(source_latents, height, width, self.vae_scale_factor)
                # import pdb;pdb.set_trace()
                latents_unpack = latents_unpack * mask_latents + source_latents_unpack * (1 - mask_latents)
                latents = self._pack_latents(latents_unpack, batch_size, num_channels_latents, 
                                             2 * (int(height) // (self.vae_scale_factor * 2)), 
                                             2 * (int(width) // (self.vae_scale_factor * 2)))
                latents = latents.to(source_latents.dtype)
                ########################################
