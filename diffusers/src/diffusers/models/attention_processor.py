...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        if attn.norm_q is not None:
            #shape: torch.Size([1, 24, 4608, 128])
            query = attn.norm_q(query)
        if attn.norm_k is not None:
            #shape: torch.Size([1, 24, 4608, 128])
            key = attn.norm_k(key)
        ...
        if encoder_hidden_states is not None:
            ...
            # attention
            #query torch.Size([1, 24, 5120, 128]); encoder_hidden_states_query_proj torch.Size([1, 24, 512, 128])
        ...
        if token_reweight is False:
            ...
            txt_len = 512
            img_len = int((attn_weight.shape[2]-txt_len) // 2)

            if encoder_hidden_states is not None:
                demo_img = torch.matmul(attn_weight[:, :, img_len+txt_len:, txt_len:txt_len+img_len], value[:, :, txt_len:txt_len+img_len, :])
                demo_txt = torch.matmul(attn_weight[:, :, img_len+txt_len:, :txt_len], value[:, :, :txt_len,:])
                hidden_states = attn_weight @ value
                hidden_states[:, :, img_len+txt_len:, :] += 0.3*demo_img
